...
def read_actions(status: str | None = None, department: str | None = None, search: str | None = None, db: Session = Depends(get_db)):
    return list_actions(db, status=status, department=department, search=search)
...
def verify(action_id: int, payload: VerificationRequest, db: Session = Depends(get_db)):
